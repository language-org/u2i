...
class Prediction:
    ...
    def _run_cluster_label_mode(
        ...
        """Assign as prediction the most frequent label associated with each cluster
        ...
        true_labels = corpus.loc[clustered.index]["category"]
        ...
        for ix, this_label in enumerate(unique_labels):
            ...
            this_label_ix = np.where(clustered == this_label)[0]
            nb_label = len(this_label_ix)

            # find its most frequent true label
            predicted = Counter(
                clustered["true_labels"].iloc[this_label_ix].tolist()
            ).most_common()[0]
            ...
            proba_predicted_all += [predicted[1] / n_clustered] * nb_label
        clustered["predicted"] = predicted_labels_all
        clustered["proba_predicted (ratio)"] = proba_predicted_all
        return clustered
    ...
    def run(self, corpus, clustered):
        ...
        try:
            if self.method == "cluster_label_mode":
                predictions = self._run_cluster_label_mode(corpus, clustered)
        except:
            raise ValueError(
