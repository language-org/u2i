...
def config_mflow(prms: dict):
    ...
    client = mlflow.tracking.MlflowClient(tracking_uri=prms["mlflow"]["tracking_uri"])

    # create or get an existing experiment
    if client.get_experiment_by_name(prms["mlflow"]["experiment_name"]) is None:
        exp_id = client.create_experiment(prms["mlflow"]["experiment_name"])
        exp = client.get_experiment(exp_id)
    else:
        exp = client.get_experiment_by_name(prms["mlflow"]["experiment_name"])
    client.set_experiment_tag(exp.experiment_id, "topic", "intent")

    # create run
    run = client.create_run(exp.experiment_id)
    client.set_tag(run.info.run_id, "model", "hierarchical clustering")
