...
DIST_THRES = 1.8  # inference threshold for clustering, low values -> more clusters
# %% [markdown]
# LOAD DATA
...
sim_ranked = similarity.rank_nearest_to_seed(
    sim_matx, seed=SEED, verbose=True
)
...
test_run.test_posting_list(
    posting_list, sim_matx, seed=SEED
)
...
cfg_mood.merge(
    todf(intents, index=filtered.index),
    left_index=True,
    right_index=True,
...
# %%
filtered_corpus = preprocess.filter_words_not_in_wordnet(
    tuple(cfg_mood["VP"])
)
# %% [markdown]
## INFERENCE
